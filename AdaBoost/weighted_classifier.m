...
p2c = ceil((bsxfun(@rdivide,bsxfun(@minus,r2,minr),(maxr-minr)))*(nthre-1)+1+1e-9);
p2c(p2c>nthre) = nthre;

p1f = floor((bsxfun(@rdivide,bsxfun(@minus,r1,minr),(maxr-minr)))*(nthre-1)+1-1e-9);  
p1f(p1f<1) = 1;

ndims=size(datafeatures,2);
i1 = repmat(1:ndims,size(p1f,1),1);  
i2 = repmat(1:ndims,size(p2c,1),1);
h1f = accumarray([p1f(:) i1(:)],repmat(w1(:),ndims,1),[nthre ndims],[],0);
h2c = accumarray([p2c(:) i2(:)],repmat(w2(:),ndims,1),[nthre ndims],[],0);
...
% We want the threshold value and dimension with the minimum error
...
dir = A(i,3); 
ind = A(i,4);
thresholds = linspace(minr(dim),maxr(dim),nthre);
thr = thresholds(ind);
