function [H, Bins] = high_histo(data, Nbins, region)
...
for i = 1:size(data,1),
   one_data		= data(i,:);
   spacing		= linspace(region(i*2-1)-2*eps,region(i*2)+2*eps,Nbins+1);
   minSpacing	= spacing(1:end-1);
   maxSpacing	= spacing(2:end);
   
   overMin		= zeros(Nbins,length(one_data));
   underMax		= zeros(Nbins,length(one_data));
